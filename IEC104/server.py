...
class Iec60850Server:
    ...
    def __init__(self, sn: int, dev_type=9999, host='', port=2404):
        ...
        self.__loop = None      # Link to event_loop, required to add new tasks
        self.__server = None    # Link to the server is needed mainly to stop
        self.__stopped = False  # Server stop flag
        self.__tasks = []       # Task list
        self._sn = sn
        ...
        self._clients = {}
        self._commands = {      # For a list of commands for a non-standard answer see __command_executor
    ...
    ************************"""
    ...
    def load(self, file_path):  # Loading list of registers from csv file
    ...
    ************************"""
    ...
    async def _interrogation_command_answer(self, client_id, asdu):
        ...
        # Confirmation of activation of the 100th command
    ...
    async def _answer(self, client_id, asdu=None):
        if asdu is None or asdu.type_id not in s_asdu.commands:
            # If this is not a command or xs something, then send an S-packet
            await self._send_s(client_id)
        else:
            # If command, then confirmation of activation or deactivation
            tmp = []
    ...
    ************************"""
    ...
    async def __handle_client(self, reader, writer):
        ...
        if self.__stopped:  # Extreme case, check
        ...
        while True:
            ...
            if self._clients[client_id]['ssn'] >= 65535 or self._clients[client_id]['rsn'] >= 65535:
                ...
                await writer.drain()
                print('{} ******* Close connection with {}: ssn or rsn overflow. *******'.format(datetime.now(), client_id))
                break
    ...
    ************************"""
    ...
    def stop(self):
        self.__stopped = True
        self.__loop.stop()  # Just in case, since finally does not always work
        ...
        try:
            ...
            # Sometimes an error pops up in the asyncio library, instead of the list of waiters comes None
